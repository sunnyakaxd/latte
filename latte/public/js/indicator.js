...
frappe.get_indicator = function(doc, doctype) {
	...
	if (doc.__powerflow_current_state) {
		return [
		  doc.__powerflow_current_state,
		  styleColors[doc.__powerflow_style] || 'grey',
		  `__powerflow_current_state,=,${doc.__powerflow_current_state}`,
		];
	  } else if (doc.__onload && doc.__onload.__powerflow_meta) {
		const meta = doc.__onload.__powerflow_meta;
		return [
		  meta.current_state,
		  styleColors[meta.style] || 'grey',
		  `__powerflow_current_state,=,${meta.current_state}`,
		];
	}
