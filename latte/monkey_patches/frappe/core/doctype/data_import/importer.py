...
def upload(rows = None, submit_after_import=None, ignore_encoding_errors=False, no_email=True, overwrite=None,
	...
	skip_errors = True, data_import_doc=None, validate_template=False, user=None, only_validate=False):
	"""upload data"""
	...
	frappe.local.flags.log_identity = f'Data Import|{data_import_doc.name}'
	...
	# only_validate = data_import_doc.only_validate or False
		...
		for i, row in enumerate(batch):
			...
			try:
				...
				if parentfield:
					...
					if only_validate:
						# to return dict
						_import_log = parent.create_data_import_log()
						frappe.throw("For Log")
					else:
						parent.save()
				else:
					if overwrite and doc.get("name") and frappe.db.exists(doctype, doc["name"]):
						...
						if only_validate:
							# to return dict
							_import_log = original.create_data_import_log()
							frappe.throw("for log")
						else:
							original.save()
						original.save()
						...
						if not update_only:
							...
							if only_validate:
								# to return dict
								_import_log = doc.create_data_import_log()
								frappe.throw("for log")
							else:
								doc.insert()
					if attachments:
				...
				if skip_errors:
					log_idx = len(data_rows_with_error)
					...
					if only_validate:
						# check for initial rows to add labels if needed
						# not a good way
						if i in (0,1,2):
							columns_idx = 0
							# find labels
							for _i, data_row in enumerate(data_rows_with_error):
								if "Column Labels:" in data_row:
									columns_idx = _i
									break

							if columns_idx and list(_import_log.keys())[0] not in data_rows_with_error[columns_idx]:
								for key in _import_log.keys():
									data_rows_with_error[columns_idx].append(cstr(key))

						for key in _import_log.keys():
							data_rows_with_error[log_idx].append(cstr(_import_log[key]))
