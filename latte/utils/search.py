...
@frappe.whitelist()
def search_link(doctype, txt, query=None, filters=None, page_length=20, searchfield=None, reference_doctype=None, ignore_user_permissions=False, returnfield=None):
	search_widget(doctype, txt, query, searchfield=searchfield, page_length=page_length, filters=filters, reference_doctype=reference_doctype, ignore_user_permissions=ignore_user_permissions, returnfield=returnfield)
	frappe.response['results'] = build_for_autosuggest(frappe.response["values"], returnfield=returnfield)
	del frappe.response["values"]

# this is called by the search box
@frappe.whitelist()
def search_widget(doctype, txt, query=None, searchfield=None, start=0,
	page_length=10, filters=None, filter_fields=None, as_dict=False, reference_doctype=None, ignore_user_permissions=False, returnfield=None):
	...
	if query and query.split()[0].lower()!="select":
		...
		if query:
			...
			fields = []
			if returnfield:
				fields.append(returnfield)
			fields.extend(get_std_fields_list(meta, searchfield or "name"))
			if filter_fields:
...
def build_for_autosuggest(res, returnfield=None):
	results = []
	for r in res:
		out = {"value": r[0], "returnfield": r[0], "description": ", ".join(unique(cstr(d) for d in r if d)[1:])}
		results.append(out)
	return results
